...
let channel = socket.channel("servers", {})
let serverList = document.getElementById("server-list");

channel.on("update", payload => {
  let serverItem = document.getElementById(`server-${payload.id}`);

  if (serverItem === null) {
    serverItem = document.createElement("li");
    serverItem.id = `server-${payload.id}`;

    serverList.appendChild(serverItem);
  }

  serverItem.innerHTML = `<span class="hero-server-solid"></span> ${payload.name} - Status: ${payload.status}`;
});
